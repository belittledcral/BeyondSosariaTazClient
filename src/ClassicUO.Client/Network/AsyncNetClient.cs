...
namespace ClassicUO.Network
{
    ...
    public sealed class AsyncNetClient : IDisposable
    {
        ...
        private readonly SemaphoreSlim _sendSignal = new SemaphoreSlim(0, int.MaxValue);
...
#nullable disable
        ...
        private async Task NetworkLoopAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested && IsConnected)
            {
                try
                {
                    // Wait until data is enqueued or timeout for stats update
                    await _sendSignal.WaitAsync(50, cancellationToken);
        ...
        private async Task ProcessSendAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                lock (_sendStream)
                {
                    ...
                    {
                        sendingBuffer = ArrayPool<byte>.Shared.Rent(4096);
