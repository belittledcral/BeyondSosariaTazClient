...
internal struct ContainerItemData
{
    public uint Serial;
...
{
    public static void AddItemsToContainerBatch(
        World world,
        ReadOnlySpan<ContainerItemData> items
    )
    {
        if (items.Length == 0)
            return;

        // Phase 1: data-model work per item
        // Track items per container for phase 2 gump updates
        Dictionary<uint, List<Item>> itemsByContainer = null;

        for (int i = 0; i < items.Length; i++)
        {
            ref readonly var data = ref items[i];

            if (Client.Game.UO.GameCursor.ItemHold.Serial == data.Serial)
                if (Client.Game.UO.GameCursor.ItemHold.Dropped)
                    Client.Game.UO.GameCursor.ItemHold.Clear();

            Entity container = world.Get(data.ContainerSerial);

            if (container == null)
            {
                Log.Warn($"No container ({data.ContainerSerial}) found");
                continue;
            }

            Item item = world.Items.Get(data.Serial);

            if (SerialHelper.IsMobile(data.Serial))
            {
                world.RemoveMobile(data.Serial, true);
                Log.Warn("AddItemToContainer function adds mobile as Item");
            }

            if (item != null && item.Container != data.ContainerSerial &&
            ...
            item = world.GetOrCreateItem(data.Serial);
            item.Graphic = data.Graphic;
            item.CheckGraphicChange();
            item.Amount = data.Amount;
            item.FixHue(data.Hue);
            item.X = data.X;
            item.Y = data.Y;
            item.Z = 0;

            if (item.Container != data.ContainerSerial)
            {
                world.RemoveItemFromContainer(item);
                item.Container = data.ContainerSerial;
            }
            ...
            if (SerialHelper.IsItem(data.ContainerSerial))
                ItemDatabaseManager.Instance.AddOrUpdateItem(item, world);

            itemsByContainer ??= new Dictionary<uint, List<Item>>();
            if (!itemsByContainer.TryGetValue(data.ContainerSerial, out var list))
            {
                list = new List<Item>();
                itemsByContainer[data.ContainerSerial] = list;
            }
            list.Add(item);
        }

        // Phase 2: one round of gump lookups per unique container
        if (itemsByContainer == null)
            return;

        foreach (var (containerSerial, containerItems) in itemsByContainer)
        {
            ...
            else if (SerialHelper.IsItem(containerSerial))
            {
                ...
                else
                {
                    ...
                    if (gump == null)
                    {
                        ContainerGump containerGump = UIManager.GetGump<ContainerGump>(containerSerial);

                        if (containerGump != null)
                        {
                            foreach (var item in containerItems)
                                containerGump.CheckItemControlPosition(item);
                        }
                    ...
                    if (gump != null)
                    {
                        if (SerialHelper.IsItem(containerSerial) && container is Item containerItem)
                            containerItem.Opened = true;
