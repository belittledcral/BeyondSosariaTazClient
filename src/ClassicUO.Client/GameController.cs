...
namespace ClassicUO
{
    internal unsafe class GameController : Microsoft.Xna.Framework.Game
    {
        ...
        private const int MAX_PACKETS_PER_FRAME = 100;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            if (useRenderTarget)
            {
                ...
                // Reset sampler back to PointClamp so next frame's rendering
                // doesn't use AnisotropicClamp (which bleeds atlas seams in tiled textures)
                if(RenderScale != 1.0f)
                {
                    _uoSpriteBatch.SetSampler(SamplerState.PointClamp);
                }
                _uoSpriteBatch.End();
            }
            else
