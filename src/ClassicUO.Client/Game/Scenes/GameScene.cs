...
namespace ClassicUO.Game.Scenes
{
    public partial class GameScene : Scene
    {
        ...
        private void UpdateTextServerEntities<T>(IEnumerable<T> entities, bool force)
            where T : Entity
        {
            int maxDist = Math.Max(Camera.Bounds.Width, Camera.Bounds.Height) / 22 + 2;
        ...
        public override void Update()
        {
            ...
            Profiler.EnterContext("WalkableManager");
            WalkableManager.Instance.Update();
            Profiler.ExitContext("WalkableManager");

            // Update LongDistancePathfinder
            Profiler.EnterContext("LongPathfinder");
            LongDistancePathfinder.Update();
            Profiler.ExitContext("LongPathfinder");

            Profiler.EnterContext("CliLoc");
            SharedStore.SendMegaCliLocRequests(_world);
            Profiler.ExitContext("CliLoc");
            ...
            Profiler.EnterContext("World.Update");
            _world.Update();
            Profiler.ExitContext("World.Update");

            Profiler.EnterContext("AnimatedStatics");
            _animatedStaticsManager.Process();
            Profiler.ExitContext("AnimatedStatics");

            Profiler.EnterContext("BoatMoving");
            _world.BoatMovingManager.Update();
            Profiler.ExitContext("BoatMoving");

            Profiler.EnterContext("Pathfinder");
            _world.Player.Pathfinder.ProcessAutoWalk();
            Profiler.ExitContext("Pathfinder");

            Profiler.EnterContext("DelayedClick");
            _world.DelayedObjectClickManager.Update();
            Profiler.ExitContext("DelayedClick");
            ...
            Profiler.EnterContext("ObjectActionQueue");
            ObjectActionQueue.Instance.Update();
            Profiler.ExitContext("ObjectActionQueue");

            Profiler.EnterContext("AutoLoot");
            AutoLootManager.Instance.Update();
            Profiler.ExitContext("AutoLoot");

            Profiler.EnterContext("GridHighlight");
            GridHighlightData.ProcessQueue(_world);
            Profiler.ExitContext("GridHighlight");
            ...
            Profiler.EnterContext("Macros");
            _world.Macros.Update();
            Profiler.ExitContext("Macros");
