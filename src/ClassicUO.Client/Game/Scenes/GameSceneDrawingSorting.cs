...
namespace ClassicUO.Game.Scenes
{
    public partial class GameScene
    {
        ...
        private readonly Dictionary<long, bool> _surfaceOverheadCache = new();
        ...
        private bool HasSurfaceOverhead(Entity obj)
        {
            ...
            long cacheKey = (long)(((ulong)obj.X << 32) | ((ulong)obj.Y << 16) | (byte)obj.Z);
            if (_surfaceOverheadCache.TryGetValue(cacheKey, out bool cached))
                return cached;
            ...
            for (int y = -1; y <= 2; ++y)
            {
                for (int x = -1; x <= 2; ++x)
                {
                    ...
                    if (!found)
                    {
                        _surfaceOverheadCache[cacheKey] = false;
                        return false;
                    }
                }
            }

            _surfaceOverheadCache[cacheKey] = found;
            return found;
        }
