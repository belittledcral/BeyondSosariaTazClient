...
namespace ClassicUO.Game.UI
{
    public class NearbyLootGump : Gump
    {
        ...
        private void UpdateNearbyLoot()
        {
            ...
            foreach (Item corpse in World.GetCorpseSnapshot())
            {
                if (!corpse.IsDestroyed && corpse.Distance <= ProfileManager.CurrentProfile.AutoOpenCorpseRange)
                {
                    ProcessCorpse(corpse, ref finalItemList);
                }
            }

            finalItemList.Sort((a, b) =>
            {
                int cmp = b.IsCoin.CompareTo(a.IsCoin); // Coins first
                if (cmp != 0) return cmp;
                cmp = a.Graphic.CompareTo(b.Graphic);
