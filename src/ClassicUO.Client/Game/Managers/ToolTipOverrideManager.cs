...
namespace ClassicUO.Game.Managers
{
    ...
    {
        private static ToolTipOverrideData[] _cachedOverrides;
        private static int _cachedOverrideCount = -1;

        public static void InvalidateCache()
        {
            _cachedOverrides = null;
            _cachedOverrideCount = -1;
        }
        ...
        public static ToolTipOverrideData[] GetAllToolTipOverrides()
        {
            ...
            int currentCount = ProfileManager.CurrentProfile.ToolTipOverride_SearchText.Count;

            if (_cachedOverrides != null && _cachedOverrideCount == currentCount)
                return _cachedOverrides;

            var result = new ToolTipOverrideData[currentCount];
            ...
            _cachedOverrides = result;
            _cachedOverrideCount = currentCount;
        ...
        private static bool MatchItemName(string itemName, string match)
        {
            ...
            if (match.StartsWith("$") && match.Length > 1)
            {
                try
                {
                    return RegexHelper.GetRegex(match.Substring(1)).IsMatch(itemName);
                }
                catch
                {
