...
namespace ClassicUO.Renderer
{
    public sealed unsafe class UltimaBatcher2D : IDisposable, IFontStashRenderer
    {
        ...
        private Vector2 _lastTexelSize;
        ...
        private void InternalDraw(Texture texture, int baseSprite, int batchSize)
        {
            ...
            {
                Vector2 texelSize = new Vector2(1f / tex2d.Width, 1f / tex2d.Height);
                if (texelSize != _lastTexelSize)
                {
                    _lastTexelSize = texelSize;
                    _basicUOEffect.TexelSize.SetValue(texelSize);
                    _basicUOEffect.Pass.Apply();
                }
            }
