...
namespace ClassicUO.Renderer.Arts
{
    ...
    {
        private const int MAX_TEXTURE_UPLOADS_PER_FRAME = 10;
        ...
        public void ResetUploadBudget() => _uploadsThisFrame = 0;
        ...
        private ref readonly SpriteInfo Get(uint idx)
        {
            ...
            {
                if (_uploadsThisFrame >= MAX_TEXTURE_UPLOADS_PER_FRAME)
                    return ref SpriteInfo.Empty;

                _uploadsThisFrame++;

                ArtInfo artInfo = PNGLoader.Instance.LoadArtTexture(idx);
                bool loadedFromPNG = !artInfo.Pixels.IsEmpty;
